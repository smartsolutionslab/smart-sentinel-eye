...
- **The stack:** 9 context services + `api-gateway` (YARP, ADR-0106) + `MigrationRunner` (gates services, ADR-0067) + containers: postgres (timescaledb), rabbitmq, keycloak (realm import), mediamtx (SFU), mosquitto, storage (Azurite emulator, ADR-0155 — replaced minio).
...
- **CI** (`.github/workflows/ci.yml`): jobs `backend`, `frontend` (parallel), `integration-shards` (needs backend; 4-way matrix, **blocking**), `integration-shard-coverage` (needs backend; **blocking**), `integration` (needs integration-shard-coverage and integration-shards; **blocking** — the stable synthesis name for the four shards), `e2e-shards` (needs backend and frontend; 4-way matrix, **blocking**; boots the full stack in run mode via `scripts/wait-for-e2e-stack.sh`), `e2e-shard-coverage` (no needs -- `--list` only, no build; **blocking**), `e2e` (needs e2e-shards and e2e-shard-coverage; **blocking**, same synthesis pattern). Actions pinned to commit SHAs; SDK pinned via `global.json`. Spec 218: transfer-based build-artifact reuse across jobs was tried and measured net-negative on the critical path (the upload cost outweighed the shard savings) — each shard restores and builds its own copy, same as before sharding. Transfer-based caches (Docker images, artifacts from a different commit) don't pay off here — favour eliminating redundant work.
