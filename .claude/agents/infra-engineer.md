...
- **CI** (`.github/workflows/ci.yml`): jobs `backend`, `frontend` (parallel), `integration-shards` (needs backend; 4-way matrix, **blocking**), `integration-shard-coverage` (needs backend; **blocking**), `integration` (needs integration-shard-coverage and integration-shards; **blocking** — the stable synthesis name for the four shards), `e2e-shards` (needs backend and frontend; 4-way matrix, **blocking**; boots the full stack in run mode via `scripts/wait-for-e2e-stack.sh`), `e2e` (needs e2e-shards; **blocking**, same synthesis pattern). Actions pinned to commit SHAs; SDK pinned via `global.json`. Spec 218: `backend` uploads its Release build as the `release-build-output` artifact; `integration-shards`, `integration-shard-coverage` and `e2e-shards` download it instead of each restoring and rebuilding the same commit — a same-run reuse. Cross-run transfer-based caching (Docker images, artifacts from a different commit) still doesn't pay off here — favour eliminating redundant work.
