...
- **Auth at trust boundaries only.** Per-service JWT validation via `ServiceDefaults.AddBearerAuthentication`; scope policies via `RequireScope`/`Scope.*`. No legacy bundle grandfathers a policy — `sse.management` was withdrawn (spec 265 / #2486) and satisfies none of them. No drive-by error handling.
