...
- **Auth (ADR-0080):** `react-oidc-context`; each app has its own `apps/*/src/app/auth.ts`, which reads `VITE_KEYCLOAK_URL` (realm `smart-sentinel-eye`). **The two apps do not share a client, and their default-granted scopes differ.** management-web uses `management-web` with **`openid` alone** — its twenty-one granular `sse.*` scopes and `sse-groups` are *default* client scopes, applied whether or not they are asked for (spec 200, issue #2279; the retired `smart-sentinel-eye-web` client and its `sse.management` grandfathered bundle have no consumer left in the repository). kiosk-web uses `kiosk-web` with **`openid` alone** — its six `sse.*` read scopes and `sse-groups` are *default* client scopes, applied whether or not they are asked for, and naming a scope the client does not hold fails the entire sign-in with `invalid_scope` (spec 041). This realm defines its own `clientScopes`, so Keycloak's built-in `basic`/`profile`/`email`/`roles` **do not exist** — any claim they would normally supply needs a mapper. The `AuthGate` in `App.tsx` registers the token getter **synchronously during render** (`setAccessTokenProvider`) — never in a `useEffect`, which races the first query into a 401.
- **UI:** Radix headless primitives + custom design system; **Tailwind with design tokens** via CSS custom properties (ADR-0077/0078). Forms: **React Hook Form + Zod** (ADR-0079). Realtime (SignalR, ADR-0152) and WebRTC media stay **direct**, off the gateway.
