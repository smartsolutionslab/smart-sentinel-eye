...
- **Scopes are `sse.<resource>.<verb>`**, catalogued in `src/ServiceDefaults/Authorization/Scope.cs` — the single source of truth. Every endpoint carries `.RequireScope(Scope.Sse.<Resource>.<Verb>)`.
  ...
  - **`sse.management` was a legacy bundle** that used to grandfather every granular `sse.*` policy. It was withdrawn (spec 265 / #2486): no policy honours it, no code path treats it as anything but an unknown string, and it is absent from the realm's scope catalogue. When reviewing a new endpoint, confirm the granular scope is what's actually being tested.
