...
- [x] T028 Walk [quickstart.md](./quickstart.md) end to end and record the observations on the PR. **"Done" is the observations.** Step 1 is the one that cannot be faked: blank the fabs, restart, and confirm streams land in *their own* fabs — if everything lands in munich, the derivation silently fell back to a default.
- [x] T029 Comment on #1155 that StreamDistribution is no longer among the contexts missing the guard, and on #1397 that only LayoutComposition remains. **Write `Closes #N, closes #M`** — the keyword must precede each number, and it only fires on merge to the default branch. Both traps caught spec 015.
