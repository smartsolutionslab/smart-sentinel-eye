...
- [x] **T001** Give `tests/LayoutComposition.Application.Tests/Fakes/FakeClock.cs` an
  `Advance(TimeSpan)` so a test can put time *inside* an awaited call. Existing
  construct-and-read call sites are unaffected.
- [x] **T002** Give `tests/LayoutComposition.Application.Tests/Fakes/RecordingLatencyBudget.cs`
...
- [x] **T003** `ResolvedOverlayTextChangedV1HandlerTests`: the recorded span
...
- [x] **T007** Run all four affected projects; capture the failures verbatim into
  `verification.md`.

## Phase 4b — implementation, RED → GREEN

- [x] **T008** `SetVariableValueCommand` gains `Option<DateTimeOffset> RootIngestedAt`.
- [x] **T009** `Variable.SetValue` takes it and stamps it on
  `VariableValueChangedDomainEvent`; the domain event gains the component.
- [x] **T010** `SetVariableValueCommandHandler` deconstructs and forwards it.
- [x] **T011** `VariableValueChangedDomainEventHandler` fills `EventMetadata`'s
  fifth argument on `ResolvedOverlayTextChangedV1`.
- [x] **T012** `SystemVariableValueRequestedV1Handler` passes the root on the
  command and **drops** `ILatencyBudget`.
- [x] **T013** `ResolvedOverlayTextChangedV1Handler` takes `ILatencyBudget` and
  records **after** the broadcast, matching `OverlayHighlightRequestedV1Handler.cs:49`.
- [x] **T014** Register nothing new — `ILatencyBudget` is already a singleton in
  `WolverineDefaults`; confirm LayoutComposition's handler resolves.
- [x] **T015** Update the two XML doc comments that define what this number means
  (`ILatencyBudget`, `LatencySegment.EventToOverlayState`) with the step change.
- [x] **T016** Counterfactual: temporarily move the recording above the broadcast
