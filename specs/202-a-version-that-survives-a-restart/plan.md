...
| R5 | Someone reintroduces a process-lifetime version cache later "for speed". | The restart integration test fails immediately — it restarts the real service, so a cache that survives in-process does not survive it. |
