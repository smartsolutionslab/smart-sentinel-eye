...
- **FR-012**: A camera's **name** MUST NOT be changeable by this feature. The change operation carries no name, so the refusal is that there is nothing to express rather than a validation that could be relaxed by accident. Tracked as #1850, which has to settle whether names are mutable *anywhere* in this product before Camera gets an exception.
  ...
  > **Overturned 2026-08-24 by spec 033** (`specs/033-rename-convention/`, ADR-0120). This requirement scoped a rename out of *this* feature, and it did that correctly — but it should not be read as a standing prohibition, which is what it looks like on its own.
  >
  > #1850 was settled, and not the way this spec's Assumptions predicted. The reasoning below rests on "no aggregate supports renaming, so names are immutable everywhere", and the addressing evidence undercuts it: `Rule` and `Variable` are addressed **by name**, while `Camera`, `Layout` and `Overlay` are addressed by an identifier. Renaming is an ordinary attribute edit in the second group and an identity change in the first, so the uniform absence of renaming is explained by the hard cases rather than by a decision about the easy ones.
  >
  > A camera can now be renamed. What that needed is exactly what this requirement listed — a per-fab case-insensitive re-check, a conflict answer distinct from the version conflict, and a ruling on the freed name — which is why deferring it here was the right call rather than a missed one.
