...
- [x] T001 Add `listAllCameraChoices` to `apps/shared/src/api/cameras.api.ts` as a `queryFn` that pages internally and returns one result: `{ items, count, complete }`. Request `sort=name`, `order=asc`, `limit=200`, walking `offset`. **`complete` is carried rather than left to each consumer to work out** (data-model.md). *Corrected during implementation: this task claimed the producer knew something the consumer could not see. It does not — completeness is exactly `items.length >= count`. Requiring more than that was the defect T005 found. It stays carried so the rule lives in one place, which is a weaker and true reason.*
