...
- [ ] T008 [P] [US1] Wire `ArchiveConfirmation` into `apps/management-web/src/features/rules/RulesPage.tsx` — the Archive button sets the pending rule; confirming calls `archiveRule` with exactly the arguments it passes today
- [ ] T009 [P] [US1] Same in `apps/management-web/src/features/systemVariables/SystemVariablesPage.tsx` for `archiveVariable`
- [ ] T010 [P] [US1] Same in `apps/management-web/src/features/layouts/LayoutsPage.tsx` for `archiveRevision`, carrying `newest.revisionNumber` and `newest.state` into the pending subject so T007's conditional has what it needs
- [ ] T011 [P] [US1] Same in `apps/management-web/src/features/overlays/OverlaysPage.tsx` for `archiveOverlayRevision`
