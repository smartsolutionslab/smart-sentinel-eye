...
5. **Whether an equivalent proxy sits in front of `stream-distribution` in production is unverified, and unverifiable until a production deployment exists.** Confirmed during T004's rework: in **run mode**, `stream-distribution`'s HTTP endpoint is reached through Aspire's DCP proxy (`WithHttpEndpoint()`, default-proxied), which terminates every inbound connection and re-originates it from its own loopback socket — so `RemoteIpAddress` as Kestrel sees it is genuinely the direct socket peer (assumption 4 still holds), but in this dev/test topology that peer is the *proxy*, not MediaMTX or a test client. This is what makes the two "sources" `WhepAuthorizeRateLimitTests` originally tried to construct indistinguishable at the layer the limiter partitions on (`WhepAuthorizeRateLimitTests.cs`, the `Authorize_partitions_the_rate_limiter_by_remote_address_not_a_global_bucket` remarks). This repository has no production deployment — the Aspire k8s publisher has never been run and no k8s package is referenced (CLAUDE.md §What lives where; `specs/047-the-decisions-we-made/audit.md:373`, issue #1015) — so there is currently no way to observe whether a k3s deployment would put an equivalent proxy (an ingress, a service-mesh sidecar) in front of `stream-distribution`, or whether MediaMTX and StreamDistribution would communicate pod-to-pod without one. **Neither is assumed here.** Should a production deployment materialise, verifying partition exactness there — that `RemoteIpAddress` is genuinely MediaMTX's own address rather than a shared proxy's — is a separate, later concern, not this spec's to resolve (mirrors spec 206 / #2429's handling of the same "no production deployment exists yet" gap for a different question, `specs/206-a-row-no-timeline-can-reach/spec.md:269`). **The consequence, not just the precondition, stated:** should a future deployment put a shared proxy in front of `stream-distribution` after all, this limiter's per-source partition collapses into exactly the global bucket §*Partition key* rejects — every caller behind that proxy, MediaMTX included, would share one bucket, and an attacker reaching it could then exhaust MediaMTX's own budget and deny it. Phase 6 review found this is not hypothetical even today: the gateway's `stream-distribution` catch-all proxied this exact anonymous route before this revision removed it (BLOCKER 1, phase-6 security review) — the same collapse this assumption describes for a *future* production proxy was, until that fix, already live through the gateway in every environment this repo boots.

## The gateway must not proxy this route — BLOCKER 1, phase-6 security review

**Removed rather than merely flagged.** `src/ApiGateway/appsettings.json`'s `stream-distribution` route is a catch-all (`/stream-distribution/{**catch-all}`), so `POST /stream-distribution/streams/authorize` was proxied straight to the `AllowAnonymous` mapping — no credential, no gateway-level auth — from the externally-reachable gateway. Combined with the DCP-proxy collapse assumption 5 already documents, an anonymous off-box caller reaching the gateway could exhaust the `whep-authorize` partition MediaMTX's own legitimate calls share, refusing MediaMTX too: the CPU-exhaustion fix this spec exists to ship would have been a DoS lever pointed at every wall. No legitimate caller uses this path through the gateway — confirmed by grepping `apps/` for `authorize`: the browser negotiates WHEP directly with MediaMTX, and MediaMTX calls `stream-distribution` directly (`mediamtx.yml`), never through the gateway. `src/ApiGateway/Program.cs` now maps `POST /stream-distribution/streams/authorize` to an explicit `404` ahead of the reverse-proxy registration — a literal route ASP.NET Core's routing prefers over the catch-all regardless of registration order — while the other `stream-distribution` endpoints the management console legitimately calls through the gateway (`GET /streams/{id}`, `GET /streams/`, `POST /streams/kiosk-latency`) stay proxied. Covered by `GatewayRoutingIntegrationTests.Gateway_does_not_forward_the_anonymous_whep_authorize_hook`.
