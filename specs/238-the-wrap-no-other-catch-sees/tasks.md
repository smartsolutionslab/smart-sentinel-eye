...
- [x] **T001** [US1] Add the connection-interceptor harness and T1 + T2 per plan §2–§3, and one
...
- [x] **T002** [US1] Run `dotnet test tests/ServiceDefaults.Tests --filter FullyQualifiedName~OutboxBacklogHealthCheckTests`
...
- [x] **T003** [US1] Counterfactuals per plan §4: (1) T2's SQLSTATE → `57P03`, run T002's command,
...
- [x] **T004** [US1] `dotnet build -c Release` (analyzers as errors) and `dotnet format --verify-no-changes`
...
- [x] **T005** [P] File O2 (composed health-check registration guard) per plan §6; no `agent:ready`;
  add to Project #13. Filed as issue #2571.
- [x] **T006** [P] File O1 (suspected `KEYCLOAK_UNAVAILABLE` misattribution,
  `RotateWebhookClientCommandHandler.cs:139`) per plan §6; board-checked first; add to Project #13.
  Filed as issue #2570.
- [x] **T007** [P] Board gate: verify #2435 on Project #13 by `content.url`, `--limit 2000`.
