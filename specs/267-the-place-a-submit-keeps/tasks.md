...
- [ ] **T000** Run `pnpm install` in the worktree (there is no `node_modules` yet). Re-read the
...
- [ ] **T000b** Re-check that the spec number 267 is free across every remote branch and worktree
  (`git ls-tree -d --name-only origin/<b>:specs`, `git worktree list`).

## Phase 4a: tests first (test-writer; run, return verbatim output)

- [ ] **T001** [P] [US1] Plan §4 cases (a)(b)(c) for Correct the address. Make the mocked
...
- [ ] **T007** [P] [US3] Plan §4's ConfirmDialog cases (a)(b)(c). The existing "Refuses a second
...
- [ ] **T008** [US1][US2][US3] **New file**: plan §6. Use the file-local `holdWrites` helper and
...
- [ ] **T009** On unchanged production code, run the following and capture the output
  **verbatim**:
...
- [ ] **T010a** [P] [US1] Plan §3.1. File:
  `apps/management-web/src/features/cameras/EditCameraAddressDialog.tsx`. Depends on T009.
- [ ] **T010b** [P] [US1] Plan §3.2. File:
  `apps/management-web/src/features/cameras/RenameCameraDialog.tsx`. Depends on T009.
- [ ] **T010c** [P] [US1] Plan §3.3. File:
  `apps/management-web/src/features/cameras/RegisterCameraDialog.tsx`. Depends on T009.
- [ ] **T010d** [P] [US1] Plan §3.4. File: `apps/management-web/src/features/rules/RuleDialog.tsx`.
  Depends on T009.
- [ ] **T010e** [P] [US1] Plan §3.5. File:
  `apps/management-web/src/features/systemVariables/SystemVariableDialog.tsx`. Depends on T009.
- [ ] **T010f** [P] [US2] Plan §3.6. File: `apps/management-web/src/features/rules/DryRunPanel.tsx`.
  Depends on T009.
- [ ] **T010g** [P] [US3] Plan §3.7 and §3.8. File: `apps/shared/src/ui/primitives/ConfirmDialog.tsx`.
...
- [ ] **T011** Re-run T009's commands. Every test must be green and **unmodified** since T009's
...
- [ ] **T012** Search again for the defect class and confirm nothing in scope was missed:
...
- [ ] **T013** **Counterfactuals** (ADR-0151 Implementation Notes). Quote each, then revert it.
