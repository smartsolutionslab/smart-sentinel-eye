...
access-token lifespan bounds that window: `accessTokenLifespan: 3600`
(`src/AppHost/Realms/smart-sentinel-eye-realm.json:5`), 1 hour, with no per-client
override for webhook clients — so the window is **≤ 3600 s**. Closing it needs a
request-time check, which is #2241's (§3).
...
| **Rotate after revoke.** Identity's rotate does not know EventIngestion revoked the integration. A *first* rotation of an already-revoked, never-rotated integration creates a fresh, enabled client. A rotation of a revoked integration whose client this spec disabled does **not** hit `RegisteredClient.Rotate`'s `InvalidOperationException`: `RotateWebhookClientCommandHandler`'s lookup is `GetWithinFabAsync` (`:57`), which excludes Disabled rows, so `existing.HasValue` is `false` and the handler never loads the aggregate at all. It takes the **create** branch instead, which calls `keycloak.CreateClientAsync` (`:126`) — and the disabled Keycloak client still exists in Keycloak (only locally marked disabled), so Keycloak answers with a conflict, which the client wrapper surfaces as `KeycloakClientAlreadyExistsException`. That is not excluded from the handler's catch clause (`:145-151`), so it is mapped to the generic `RotateWebhookClientFailures.KeycloakUnavailable` — a **502**, not a 409/`InvalidOperationException`. The operator gets a misleading "Keycloak unavailable" response rather than a clean conflict, which makes this gap slightly worse than a 409 would be. | A follow-up issue (**to be filed at the gate**) | Needs a sse.webhooks.write operator acting deliberately, not a leaked credential. Fixing it needs Identity to hold revocation state or EventIngestion to gate rotation, which is a design choice this bug fix should not make silently. |
| **Keycloak-outage / dead-letter residual risk.** No custom Wolverine failure policy (`OnException` / `RetryWithCooldown` / `MoveToErrorQueue`) is configured for `WebhookIntegrationRevokedIntegrationEventHandler` or anywhere else on this path. If Keycloak is down for longer than Wolverine's default retry budget, `WebhookIntegrationRevokedV1` eventually dead-letters on Identity's error queue. The webhook client then stays enabled indefinitely, with no alerting beyond a per-attempt Warning log — while the operator who called `DELETE /webhook-integrations/{name}` already saw `200`. | Accepted risk; a follow-up issue for a custom failure policy, if wanted (**not this PR**) | Adding retry/cooldown configuration is a design decision, not a bug fix, and is explicitly out of scope here (§7). Recovery today is manual: replay Identity's error queue, or disable the client by hand in the Keycloak admin console. |
