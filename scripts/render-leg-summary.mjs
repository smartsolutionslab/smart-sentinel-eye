...
function renderSection(attempts) {
  ...
  lines.push('| Attempt | Samples | p50 | max | Frame interval (before → after) |', '| --- | --- | --- | --- | --- |');
  ...
  // Phase-6 review (spec 225): the floor is 1.5*T at the median and 2*T at
  // the tail (ADR-0123:64's `elapsed ≈ (0…T) + T + work` decomposition), not
  // T itself — printing T alone lets a reader subtract T from p50 and read
  // the remainder as compositing cost, which overstates it by roughly 1.5x.
  // Computed from the most recent attempt's own frame interval so the
  // reading is anchored to a number on this same page, not a generic ratio.
  const last = attempts.find((attempt) => attempt.ok && attempt.record.count > 0);
  if (last) {
    const frameMilliseconds =
      (last.record.frameIntervalBeforeMilliseconds + last.record.frameIntervalAfterMilliseconds) / 2;
    const medianFloor = 1.5 * frameMilliseconds;
    const tailFloor = 2 * frameMilliseconds;
    lines.push(
      `The frame interval is not a background number — it is roughly the floor \`overlay_draw\` cannot beat on ` +
        'this runner (ADR-0123, `elapsed ≈ (0…T) + T + work`): a shared `ubuntu-latest` box, headless Chromium, ' +
        `software rasterisation. At attempt ${last.record.attempt}'s own interval (T ≈ ${twoDecimals(frameMilliseconds)} ms), ` +
        `the arithmetic floor is **~${oneDecimal(medianFloor)} ms at the median, ~${oneDecimal(tailFloor)} ms at the tail** ` +
        '— subtract that floor from a figure above, not T alone, before reading the remainder as compositing cost. ' +
        'Read a high figure as cadence first, compositing second.',
      '',
    );
  }
