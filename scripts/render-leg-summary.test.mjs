...
test("the cadence reading derives the 1.5x/2x floor from this run's own frame interval, not T alone (phase-6 review)", () => {
  const directory = stubDirectory();
  // T = (16.67 + 16.40) / 2 = 16.535 ms. ADR-0123's table: median floor
  // 1.5*T = 24.8 ms, tail floor 2*T = 33.1 ms.
  writeAttemptFile(
    directory,
    0,
    attemptRecord({
  ...
  assert.equal(outcome.result.status, 0, describeFailure(outcome));
  assert.match(outcome.summary, /cadence first, compositing second/i, describeFailure(outcome));
  assert.match(outcome.summary, /24\.8 ms at the median/i, describeFailure(outcome));
  assert.match(outcome.summary, /33\.1 ms at the tail/i, describeFailure(outcome));
  // The old (wrong) framing subtracted T alone; the fixed sentence must not
  // claim T itself is the floor without the 1.5x/2x multiplier attached.
  assert.doesNotMatch(outcome.summary, /is the floor `overlay_draw` cannot beat/i, describeFailure(outcome));
});
