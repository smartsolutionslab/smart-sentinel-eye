...
/// <summary>
/// When the source system observed the audited event.
///
/// <para>
...
public sealed record OccurredAt(DateTimeOffset Value) : IValueObject<DateTimeOffset>, IComparable<OccurredAt>
{
    ...
    /// access (<c>x.OccurredAt.Value</c>) does not translate and falls back to
    /// client evaluation, which passes tests while scanning the table.
    /// </summary>
    public static implicit operator DateTimeOffset(OccurredAt value) => value.Value;
