// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using Npgsql.EntityFrameworkCore.PostgreSQL.Metadata;
using SmartSentinelEye.SystemVariables.Infrastructure.Persistence;

#nullable disable

namespace SmartSentinelEye.SystemVariables.Infrastructure.Persistence.Migrations
{
    [DbContext(typeof(SystemVariablesDbContext))]
    [Migration("20260921044905_AddOverlayTextVersion")]
    partial class AddOverlayTextVersion
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "10.0.11")
                .HasAnnotation("Relational:MaxIdentifierLength", 63);

            NpgsqlModelBuilderExtensions.UseIdentityByDefaultColumns(modelBuilder);

            modelBuilder.Entity("SmartSentinelEye.SystemVariables.Domain.Variable.Variable", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uuid")
                        .HasColumnName("variable_id");

                    b.Property<string>("Fab")
                        .IsRequired()
                        .HasMaxLength(32)
                        .HasColumnType("character varying(32)")
                        .HasColumnName("fab");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("character varying(64)")
                        .HasColumnName("name");

                    b.Property<string>("State")
                        .IsRequired()
                        .HasMaxLength(16)
                        .HasColumnType("character varying(16)")
                        .HasColumnName("state");

                    b.Property<string>("Type")
                        .IsRequired()
                        .HasMaxLength(16)
                        .HasColumnType("character varying(16)")
                        .HasColumnName("type");

                    b.Property<string>("Value")
                        .IsRequired()
                        .HasMaxLength(512)
                        .HasColumnType("character varying(512)")
                        .HasColumnName("value_packed");

                    b.Property<int>("Version")
                        .IsConcurrencyToken()
                        .HasColumnType("integer")
                        .HasColumnName("version");

                    b.HasKey("Id");

                    b.HasIndex("Fab", "Name")
                        .IsUnique()
                        .HasDatabaseName("ux_system_variables_fab_name_active")
                        .HasFilter("state <> 'Archived'");

                    b.ToTable("system_variables", (string)null);
                });

            modelBuilder.Entity("SmartSentinelEye.SystemVariables.Domain.Variable.Variable", b =>
                {
                    b.OwnsOne("SmartSentinelEye.SystemVariables.Domain.Variable.BooleanLabels", "BooleanLabels", b1 =>
                        {
                            b1.Property<Guid>("VariableId")
                                .HasColumnType("uuid");

                            b1.Property<string>("FalsyLabel")
                                .IsRequired()
                                .HasMaxLength(64)
                                .HasColumnType("character varying(64)")
                                .HasColumnName("falsy_label");

                            b1.Property<string>("TruthyLabel")
                                .IsRequired()
                                .HasMaxLength(64)
                                .HasColumnType("character varying(64)")
                                .HasColumnName("truthy_label");

                            b1.HasKey("VariableId");

                            b1.ToTable("system_variables");

                            b1.WithOwner()
                                .HasForeignKey("VariableId");
                        });

                    b.OwnsOne("SmartSentinelEye.SystemVariables.Domain.Variable.Creation", "Creation", b1 =>
                        {
                            b1.Property<Guid>("VariableId")
                                .HasColumnType("uuid");

                            b1.Property<DateTimeOffset>("At")
                                .HasColumnType("timestamp with time zone")
                                .HasColumnName("created_at");

                            b1.Property<Guid>("By")
                                .HasColumnType("uuid")
                                .HasColumnName("created_by");

                            b1.HasKey("VariableId");

                            b1.ToTable("system_variables");

                            b1.WithOwner()
                                .HasForeignKey("VariableId");
                        });

                    b.Navigation("BooleanLabels");

                    b.Navigation("Creation")
                        .IsRequired();
                });
#pragma warning restore 612, 618
        }
    }
}
