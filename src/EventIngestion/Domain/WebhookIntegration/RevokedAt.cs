...
/// <summary>
/// When the integration was revoked, where it has been.
///
/// <para>
...
/// </para>
/// </summary>
public sealed record RevokedAt(DateTimeOffset Value) : IValueObject<DateTimeOffset>, IComparable<RevokedAt>
{
    /// <summary>
    ...
    /// </summary>
    public int CompareTo(RevokedAt? other) =>
        other is null ? 1 : Value.CompareTo(other.Value);

    public static bool operator <(RevokedAt left, RevokedAt right) =>
        Comparer<RevokedAt>.Default.Compare(left, right) < 0;
    ...
    public static RevokedAt From(DateTimeOffset value) =>
    ...
    /// access (<c>x.RevokedAt.Value</c>) does not translate and falls back to
    /// client evaluation, which passes tests while scanning the table.
    /// </summary>
    public static implicit operator DateTimeOffset(RevokedAt value) => value.Value;
