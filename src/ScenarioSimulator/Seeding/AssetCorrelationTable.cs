...
{
    private readonly ConcurrentDictionary<string, AssetRow> _rows = new(StringComparer.Ordinal);
    private int _wallClaimed;
    ...
    {
        ready = _rows.Values.Count(row => row.Overlay.HasValue && row.Camera.HasValue);
        return expectedCount > 0 && ready == expectedCount;
    }

    /// <summary>Wins for exactly one caller; the winner creates the wall.</summary>
    public bool TryClaimWallCreation() => Interlocked.CompareExchange(ref _wallClaimed, 1, 0) == 0;

    /// <summary>Release the claim so a later event retries (e.g. after a create failure).</summary>
    public void ReleaseWallClaim() => Interlocked.Exchange(ref _wallClaimed, 0);
