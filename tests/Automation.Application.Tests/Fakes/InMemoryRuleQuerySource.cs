...
/// <summary>
/// In-memory <see cref="IRuleQuerySource"/> for query-handler tests. Wraps the
/// repository's list in a TestAsyncEnumerable so EF Core's ToListAsync /
/// SingleOrDefaultAsync extensions resolve against an IAsyncQueryProvider
/// (same shim as the OverlayDesigner + LayoutComposition suites).
/// </summary>
public sealed class InMemoryRuleQuerySource(InMemoryRuleRepository repository) : IRuleQuerySource
{
    public IQueryable<RuleAggregate> Rules =>
        new TestAsyncEnumerable<RuleAggregate>(repository.Rules);
}
