...
public class TimestampOrderingTests
{
    ...
    {
        OrdersByItsMoment(ProvisionedAt.From);
        OrdersByItsMoment(LastSuccessAt.From);

        OperatorsAgreeWithCompareTo(ProvisionedAt.From, (a, b) => a < b, (a, b) => a > b, (a, b) => a <= b, (a, b) => a >= b);
        OperatorsAgreeWithCompareTo(LastSuccessAt.From, (a, b) => a < b, (a, b) => a > b, (a, b) => a <= b, (a, b) => a >= b);

        NormalizesWithoutMovingTheInstant(ProvisionedAt.From, x => x);
        NormalizesWithoutMovingTheInstant(LastSuccessAt.From, x => x);
    }
}
