...
public class KioskEnrolledV1Tests
{
    ...
    public void Exposes_every_field_via_the_positional_constructor()
    {
        ...
        KioskEnrolledV1 evt = new(id, "kiosk-3", "munich", Moment);
    ...
    public void Implements_IIntegrationEvent_so_Wolverine_can_route_it() =>
        new KioskEnrolledV1(Guid.CreateVersion7(), "k", "f", Moment)
    ...
    {
        Guid id = Guid.CreateVersion7();
        new KioskEnrolledV1(id, "k", "f", Moment)
            .ShouldBe(new KioskEnrolledV1(id, "k", "f", Moment));
    }

    [Fact]
    public void JSON_round_trip_preserves_every_field()
    {
        KioskEnrolledV1 original = new(Guid.CreateVersion7(), "kiosk-3", "munich", Moment);
