...
public class TimestampOrderingTests
{
    ...
    {
        OrdersByItsMoment(RejectedAt.From);
        OrdersByItsMoment(OccurredAt.From);
        OrdersByItsMoment(IngestedAt.From);
        ...
        OperatorsAgreeWithCompareTo(IngestedAt.From, (a, b) => a < b, (a, b) => a > b, (a, b) => a <= b, (a, b) => a >= b);
        ...
        NormalizesWithoutMovingTheInstant(IngestedAt.From, x => x);
