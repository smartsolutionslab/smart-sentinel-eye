...
public class TimestampOrderingTests
{
    ...
    {
        OrdersByItsMoment(CreatedAt.From);
        OrdersByItsMoment(PublishedAt.From);
        OrdersByItsMoment(ArchivedAt.From);

        OperatorsAgreeWithCompareTo(CreatedAt.From, (a, b) => a < b, (a, b) => a > b, (a, b) => a <= b, (a, b) => a >= b);
        OperatorsAgreeWithCompareTo(PublishedAt.From, (a, b) => a < b, (a, b) => a > b, (a, b) => a <= b, (a, b) => a >= b);
        OperatorsAgreeWithCompareTo(ArchivedAt.From, (a, b) => a < b, (a, b) => a > b, (a, b) => a <= b, (a, b) => a >= b);

        NormalizesWithoutMovingTheInstant(CreatedAt.From, x => x);
        NormalizesWithoutMovingTheInstant(PublishedAt.From, x => x);
        NormalizesWithoutMovingTheInstant(ArchivedAt.From, x => x);
    }
}
