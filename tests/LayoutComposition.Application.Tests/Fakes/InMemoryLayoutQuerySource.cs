...
/// <summary>
/// In-memory ILayoutQuerySource for query-handler tests. Wraps the
...
/// IAsyncQueryProvider (mirrors the StreamDistribution pattern).
/// </summary>
public sealed class InMemoryLayoutQuerySource(InMemoryLayoutRepository repository) : ILayoutQuerySource
{
    public IQueryable<Layout> Layouts =>
        new TestAsyncEnumerable<Layout>(repository.Layouts);
}
