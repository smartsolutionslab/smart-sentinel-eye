...
public class TimestampOrderingTests
{
    ...
    public void Every_instant_orders_normalizes_and_compares()
    {
        ...
        OrdersByItsMoment(DisabledAt.From);
        OrdersByItsMoment(LastRotatedAt.From);

        OperatorsAgreeWithCompareTo(RegisteredAt.From, (a, b) => a < b, (a, b) => a > b, (a, b) => a <= b, (a, b) => a >= b);
        OperatorsAgreeWithCompareTo(DisabledAt.From, (a, b) => a < b, (a, b) => a > b, (a, b) => a <= b, (a, b) => a >= b);
        OperatorsAgreeWithCompareTo(LastRotatedAt.From, (a, b) => a < b, (a, b) => a > b, (a, b) => a <= b, (a, b) => a >= b);

        NormalizesWithoutMovingTheInstant(RegisteredAt.From, x => x);
        NormalizesWithoutMovingTheInstant(DisabledAt.From, x => x);
        NormalizesWithoutMovingTheInstant(LastRotatedAt.From, x => x);
    }
}
